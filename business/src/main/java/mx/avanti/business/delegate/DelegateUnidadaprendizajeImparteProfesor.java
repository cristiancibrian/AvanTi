...
//        return respuesta; 
    ...
    public List<UnidadaprendizajeImparteProfesor> findByProfesorYPE2(int proid, int peid, String ce,int coordid) {
       //return executeProcedure("CALL buscarUapPorProfesorYPE2(:profId, :peId, :ce, :coordId)", new String[]{"proid", "peid", "ce", "coordinadorProId"}, proid, peid, ce, coordinadorProId);
        
       List<UnidadaprendizajeImparteProfesor> result = null;
       try{
        result = ServiceLocator.getInstanceUnidadAprendizajeImparteProfesor().executeProcedureText("CALL buscarUapPorProfesorYPE2( \'"+ proid  +"\' , \'"+peid+"\', \'"+ce+"\' , \'"+coordid+"\')");
         }catch(Exception e){
            System.err.println("Error en consultar asignaciones por ID" + e.getMessage());
            e.printStackTrace();
        }
