...
public class ProfesorBeanHelper implements Serializable {
    ...
    public List<Profesor> filtrado(String busqueda) {
     ...
     public Usuario findByUsuarioId(int idUsu){
         return ServiceFacadeLocator.getInstanceProfesorFacade().findByUsuarioId(idUsu);
     }
...
//                listaFiltrada = ServiceFacadeLocator.getInstanceProgramaEducativoFacade().buscarProgramaEducativoPorID(listaProgramaEducativo.get(0).getPEDid()).getProfesorList();
    ...
    public List<Profesor> filtrarTextBox(String busqueda) {
        ...
        boolean letras = false; //Bandera para saber si son letras
        if (!busqueda.equals("")) {

            try { //Si nos lanza una excepción sabemos que son letras
            ...
            for (Profesor prof : listaFiltrada2) {
                //Creamos el nombre completo para que pueda buscar al profesor por cualquier parte del nombre
                String nombreCompleto = prof.getPROnombre() + " " + prof.getPROapellidoPaterno() + " " + prof.getPROapellidoMaterno();
                if (!letras) { //Si no son letras buscaremos por el número de empleado
                    if (Integer.toString(prof.getPROnumeroEmpleado()).startsWith(busqueda)) {
                        listaFiltrada.add(prof);
                    }
                } else { //Si son letras lo podremos buscar por RFC, Nombres, apellidos y Usuario 
                    if (nombreCompleto.toLowerCase().contains(busqueda.toLowerCase())) {
                        listaFiltrada.add(prof);
                    } else if (prof.getPROrfc().toLowerCase().startsWith(busqueda.toLowerCase())) {
                        listaFiltrada.add(prof);
                    } else if (prof.getUsuarioUSUid() != null) { //Nos aseguramos que tenga un usuario para poder buscar por usuario
                        if (prof.getUsuarioUSUid().getUSUusuario().toLowerCase().startsWith(busqueda.toLowerCase())) {
                            listaFiltrada.add(prof);
                        }
                    }
                }
    ...
    public List<Profesor> ordenarProfesores(List<Profesor> profesores) {
        Comparator<Profesor> criteriosComparator = Comparator.comparing(Profesor::getPROnumeroEmpleado);
        profesores.sort(criteriosComparator);
        return profesores;
    }
