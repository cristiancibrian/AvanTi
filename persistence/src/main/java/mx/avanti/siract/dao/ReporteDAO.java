...
public class ReporteDAO extends AbstractDAO <Integer, Reporte> {
    Session session;
  //  ReporteAvanceContenidoTematicoDAO auxRACTD;
    /*
    /**
    ...
    */
    ...
    public List<Reporte> getLatest(String tipoGrupo, String ce, String ractNumero, int uipId){
        ...
        switch(tipoGrupo){
             case "C": //Clase
                //joinTipoGrupo = "unidad uni ON uni.UNIid = rep.UNIid"; ORIGINAL
                joinTipoGrupo = "unidad uni ON uni.UNIid = rep.UNIid "
                        + "left join temaunidad tema on tema.TUNid = rep.TUNid "
                        + "left join subtemaunidad subtema on subtema.SUTid = rep.SUTid ";
                //orderBy = "uni.UNInumero, rep.UNIid, rep.TUNid, rep.SUTid, ract.RACnumero"; ORIGINAL
                orderBy = "uni.UNInumero, tema.TUNnumero, subtema.SUTnumero, rep.UNIid desc";
                break;
            case "T": //Taller
    ...
    //Este metodo guarda o actualiza un reporte
     @Override
    public void saveOrUpdate(Reporte obj) {
        System.out.println("SaveOrUpdate ----------");
        HibernateUtil.getSession();
            HibernateUtil.getSession().merge(obj);
    }
    //Este metodo actualiza un reporte
    @Override
    public void update(Reporte obj) {
        System.out.println("update ----------");
    HibernateUtil.getSession();
            HibernateUtil.getSession().update(obj);
    }
    //Este metodo elimina un reporte
    @Override
    public void delete(Reporte obj) {
        System.out.println("Delete ----------");
        HibernateUtil.getSession();
            HibernateUtil.getSession().delete(obj);
    }
    
     public List<Reporte> executeProcedureTest(String procedure) {
        System.out.println("ExecuteProcedure ----------");
        List<Reporte> result;
        HibernateUtil.getSession();
        Query lQuery = HibernateUtil.getSession().createSQLQuery(procedure).addEntity(Reporte.class);
         result = lQuery.list();
         return result;
    }
